...
void I2C_LCD_Normal(void)
{
	I2C_LCD_init();
	I2C_LCD_write_string_XY(0,6,"Chill"); //개행문자 쓰지마.
	I2C_LCD_write_string_XY(1,5,"Driver "); //개행문자 쓰지마.
	_delay_ms(10);  // I2C 통신 후 잠시 대기
}

void I2C_LCD_Auto(void)
{
	I2C_LCD_init();
	I2C_LCD_write_string_XY(0,5,"Mi Chill"); //개행문자 쓰지마.
	I2C_LCD_write_string_XY(1,5,"Driver "); //개행문자 쓰지마.
}
// 1byte를 write
void I2C_LCD_write_data(uint8_t data)
